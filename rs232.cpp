...
int Cport[MAX_CONNECTOINS],
    error;

struct termios new_port_settings,
       old_port_settings[MAX_CONNECTOINS];
/*
...
int RS232_OpenComport(int comport_number, int baudrate, const char* name)
{
  int baudr, status;

  if((comport_number>(MAX_CONNECTOINS-1))||(comport_number<0))
  {
  ...
  Cport[comport_number] = open(name, O_RDWR | O_NOCTTY | O_NDELAY);
...
void RS232_startPortSearch(){
	serialsearchd = opendir("/dev/");
}

bool RS232_getNextPort(char* portName){
	struct dirent *dir;
	if (serialsearchd){
		while ((dir = readdir(serialsearchd)) != NULL){
			string dname = dir->d_name; 
			if (dname.length() >= 4) {
				bool isTTY = dname.substr(0,3) == "tty";
				bool isACM = (dname.length() >= 6) && (dname.substr(3,3) == "ACM");
				bool isAMA = (dname.length() >= 6) && (dname.substr(3,3) == "AMA");
				bool isUSB = (dname.length() >= 6) && (dname.substr(3,3) == "USB");
				bool isS = dname.substr(3,1) == "S";

				bool isRF = (dname.length() >= 6) && dname.substr(0,6) == "rfcomm";
				bool isIR = (dname.length() >= 6) && dname.substr(0,6) == "ircomm";
				if ((isTTY && (isACM||isUSB||isS)) || (isRF || isIR)){
					std::string str = "/dev/";
					str.append(dname.data());
					std::size_t length = str.copy(portName,256,0);
					portName[length]='\0';
					return true;
				}
			}
