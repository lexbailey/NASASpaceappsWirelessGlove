...
void doTheThing(){
	if (cMode == rotate){
		gloveData_t rotation = gdnorm - gstartstate;
		printf("%.3f, %.3f, %.3f\n", rotation.handGyX/32768.0f, rotation.handGyY/32768.0f, rotation.handGyZ/32768.0f);
		x = lx+((rotation.handGyX/32768.0f)-sx);
		y = ly+((rotation.handGyY/32768.0f)-sy);
		z = lz+((rotation.handGyZ/32768.0f)-sz);
		setRotation(x,y,z);
